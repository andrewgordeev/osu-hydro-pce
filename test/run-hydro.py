...
def run_hydro(fs, event_size, coarse=False, dt_ratio=0.25, profile=None):
    ...
    subprocess.run(['osu-hydro-pce', 'edec=0.1', 't0=0.1', 'teq=5.0', 'dt={}'.format(dt), 'dxy={}'.format(dxy), 'nls={}'.format(ls), 'tfinal=15.0', 'VisSlope = 0', 'VisHRG = 0.08', 'VisBulkMax = 0', 'VisMin = 0.08', 'ViscousEqsType = 1'],  #'VisSlope = 0', 'VisHRG = 0.08', 'VisBulkMax = 0', 'VisMin = 0.08' 
                  check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
    #print(subprocess.getoutput(['osu-hydro', 't0=0.1', 'teq=5.0', 'dt={}'.format(dt), 'dxy={}'.format(dxy), 'nls={}'.format(ls)]))# """ Plotting contours """
...
profile = np.load('profiles/profilex15n13p1Gauss.npy')
...
#plt.savefig("plots/TempTest.png")
...
# plt.clabel(cs, inline=0, fontsize=10)
# plt.savefig("plots/FugacityTest.png")
