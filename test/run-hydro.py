...
def run_hydro(fs, event_size, coarse=False, dt_ratio=0.25, profile=None):
    ...
    subprocess.run(['osu-hydro-pce', 'edec=0.1', 't0=0.1', 'teq=0.0', 'dt={}'.format(dt), 'dxy={}'.format(dxy), 'nls={}'.format(ls), 'tfinal=15.0', 'VisSlope = 0', 'VisHRG = 0.04', 'VisBulkMax = 0', 'VisMin = 0.04', 'ViscousEqsType = 1'],  #'VisSlope = 0', 'VisHRG = 0.08', 'VisBulkMax = 0', 'VisMin = 0.08' 
                 check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
   # print(subprocess.getoutput(['osu-hydro-pce', 'edec=0.1', 't0=0.1', 'teq=0.0', 'dt={}'.format(dt), 'dxy={}'.format(dxy), 'nls={}'.format(ls), 'tfinal=15.0', 'VisSlope = 0', 'VisHRG = 0.0', 'VisBulkMax = 0', 'VisMin = 0.0', 'ViscousEqsType = 1']))
...
profile = np.load('profiles/profileTempGauss200.npy')
...
ic = np.fromfile('ed.dat').reshape(201,201)
#plt.plot(ic[51,:])
